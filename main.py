...
def extract_players_from_socket(socket_log_path):

    pattern = re.compile(r'ON DeviceInfo:\s*(\{.*\})')
    mapping = {}

    with open(socket_log_path, encoding="utf-8") as f:
        for line in f:
...
def parse_stats_log(stats_log_path, mapping):
    ...
    with open(stats_log_path, encoding="utf-8") as f:
        for line in f:
...
def parse_logs_and_build_dataframe(socket_log, stats_log):

    mapping = extract_players_from_socket(socket_log)
    df = parse_stats_log(stats_log, mapping)
    return df